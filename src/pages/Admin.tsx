...
export function Admin() {
  ...
  const { getAllAffiliates, updateAffiliate, deleteAffiliate } = useAffiliate();
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[85vh] flex items-center justify-center bg-[#fdfcfb] px-4 relative overflow-hidden">
        {/* Decorative elements */}
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-black/10 to-transparent" />
        <div className="absolute -top-24 -left-24 w-64 h-64 bg-black/[0.02] rounded-full blur-3xl" />
        <div className="absolute -bottom-24 -right-24 w-64 h-64 bg-black/[0.02] rounded-full blur-3xl" />

        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-gray-100 p-10 relative z-10">
          <div className="text-center mb-10">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-black text-white mb-6 shadow-xl rotate-3 hover:rotate-0 transition-transform duration-500">
              <span className="font-serif text-2xl font-bold">SBC</span>
            </div>
            <h1 className="text-3xl font-serif font-bold text-gray-900 tracking-tight">Espace Admin</h1>
            <div className="w-12 h-0.5 bg-black mx-auto mt-4 mb-4 opacity-20" />
            <p className="text-gray-500 text-sm font-light">Gestion exclusive de Sama Butik</p>
          </div>
          
          <form onSubmit={handleLogin} className="space-y-6">
            {error && (
              <div className="bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-xl text-sm text-center animate-shake">
                {error}
              </div>
            )}
            
            <div className="space-y-2">
              <label className="text-[11px] uppercase tracking-[0.2em] font-semibold text-gray-400 ml-1">Utilisateur</label>
              <input 
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                className="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-black/5 focus:border-black outline-none transition-all duration-300 placeholder:text-gray-300"
            ...
            <div className="space-y-2">
              <label className="text-[11px] uppercase tracking-[0.2em] font-semibold text-gray-400 ml-1">Mot de passe</label>
              <input 
                type="password"
                value={password}
            ...
            <button 
              ...
              className="w-full bg-black text-white font-medium py-4 px-6 rounded-2xl hover:bg-gray-800 transition-all duration-300 shadow-lg shadow-black/10 active:scale-[0.98] mt-4 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoggingIn ? (
                <>
          ...
          <div className="mt-10 pt-8 border-t border-gray-50 text-center">
            <p className="text-[10px] uppercase tracking-widest text-gray-300">Sama Butik &copy; 2026</p>
          </div>
        </div>
      </div>
    );
